...
In the context of embedding search, "Top N" refers to selecting the top N results that are most similar to a query based on their [embeddings](/get-started/newtogenai#embeddings). Embeddings are numerical representations of items (like texts, images, or products) that capture their characteristics in a way that can be processed by computers.
